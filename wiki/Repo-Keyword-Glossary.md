...
| improvement artifact | 任務卡住或完成後的改善紀錄；它是偏人類閱讀的復盤文件，通常用 PDCA 方式整理根因、矯正與預防。任務若要從 `blocked` 恢復時，這份文件是必填；一般 verify / done 後則屬於建議補充。 | `docs/artifact_schema.md` §5.8、`docs/workflow_state_machine.md` |
