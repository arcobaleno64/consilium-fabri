...
| improvement artifact | 任務卡住或完成後的改善紀錄；它是偏人類閱讀的復盤文件，通常用 PDCA 方式整理根因、矯正與預防，和給 workflow automation 使用的 `status.json` 不同。 | `docs/artifact_schema.md` §5.8、`docs/workflow_state_machine.md` |
...
| Gate E | 任務如果曾 blocked，要先補一份 improvement artifact，完成恢復所需紀錄後，才能合法恢復流程；這個恢復關卡就叫 Gate E。 | `docs/workflow_state_machine.md`、`docs/artifact_schema.md` |
