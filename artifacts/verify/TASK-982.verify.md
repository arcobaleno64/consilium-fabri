...
### AC-1: `normalize_api_base_url(...)` 預設只接受 GitHub.com 與 allowlisted 自訂 host
- **Result**: PASS
...
### AC-2: guard unit tests 覆蓋 default / reject / allow 與 provider fetch 拒絕路徑
- **Result**: PASS
...
### AC-3: red-team suite 具備 allowlist 正反案例並更新 runbook
- **Result**: PASS
...
### AC-4: root / `template/` 的 guard、tests、runbook 與 README 同步完成
- **Result**: PASS
...
### AC-5: 驗證證據包含 unit tests、red-team、contract guard 與 TASK-982 status guard
- **Result**: PASS
