...
| Guard Contract Validator | artifacts/scripts/guard_contract_validator.py | Prompt / README 同步檢查 |
...
### GitHub API
- 用途：讀取 PR / issue metadata, 推送 artifact 到 release notes
...
- 同步方式：Each PR reset to upstream/<default> via `git fetch upstream && git reset --hard upstream/main`
...
| 變數 | 用途 | 示例 | 必需 |
|---|---|---|---|
| `GITHUB_TOKEN` | 驗證 GitHub API | ghp_xxx... | No（fallback 為唯讀） |
| `PYTHONPATH` | Import artifacts/scripts | `C:\...\CLI` | For guard scripts |
| `.venv/` | Python venv | Activated before running guards | Yes |
...
| Prompt regression 失敗 | CLAUDE.md 或 GEMINI.md 變了 | 執行 `guard_contract_validator` 檢查 diff |
